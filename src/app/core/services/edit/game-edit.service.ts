...
export class GameEditService {
  ...
  saveVersion(data: Version) {
    if (data.id) {
      return this.http.patch<Version>(API_URLS.VERSIONS(data.game, data.id), data);
    } else {
      return this.http.post<Version>(API_URLS.VERSIONS(data.game), data);
    }
  }
  ...
  saveSandbox(data: Version) {
    if (data.id) {
      return this.http.patch<Version>(API_URLS.SANDBOXES(data.game, data.id), data);
    } else {
      return this.http.post<Version>(API_URLS.SANDBOXES(data.game), data);
    }
  }
