...
export class PixiFactory implements AbstractFactory {
  ...
  createComponent(elem: RzElement, meta: MetaProps): BasicComponent {
    switch (elem.type) {
      ...
      case PRIMS.input:
        return this.createInput(elem as RzElement<PrimitiveInputProps>, meta);
    }
  }
  createInput(elem: RzElement<PrimitiveInputProps>, meta: MetaProps): PrimitiveInput {
    const doc = this.document;
    const input = this.document.createElement(elem.props.multiline ? 'textarea' : 'input');
    const component = new PrimitiveInput(elem.props, input, meta);
    component.container = {
