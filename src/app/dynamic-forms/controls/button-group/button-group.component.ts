...
export class ButtonGroupComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.data.multiple) {
      if (this.data.asString) {
        this.value = this.data.value ? this.data.value.split(',').filter((elem: any) => !!elem) :
  ...
  isButtonChecked(value: any) {
    if (this.value !== undefined) {

      if (this.data.multiple) {
        const field = this.data.valueField;
        const index = this.value.map((elem: any) => field ? elem[field] : elem).indexOf(value);
        return index !== -1;
      } else {
  ...
  handleChange(value: any) {
    ...
    if (this.data.multiple) {
      const asIntArr = currentValue.map((elem: any) => field ? elem[field] : elem);
